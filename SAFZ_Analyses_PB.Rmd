...
# Data subset for model with thinned data
PBDataAll<- PBSeedAll %>%
  # Remove first 7 days of track
  filter(DayOfTrip > 7) %>%
  # For complete trips, remove last 7 days of track
  group_by(TOPPID) %>%
...
# Data subset for model without thinning
PBDataAll2<- PBSeedAll %>%
  # Remove first 7 days of track
  filter(DayOfTrip > 7) %>%
  # For complete trips, remove last 7 days of track
  group_by(TOPPID) %>%
...
# Order data frame and define rows where each deployment starts for AR.start vector
PBDataAll2 <- PBDataAll2[order(PBDataAll2$TOPPID, PBDataAll2$DayOfTrip), ]
PBDataAll2$AR.start <- c(TRUE, PBDataAll2$TOPPID[-1] != PBDataAll2$TOPPID[-nrow(PBDataAll2)])
...
```{r Tune rho}
# Define size and number of coarse and fine steps around median to tests
coarse_window = 0.3
n_coarse = 7
n_fine = 9
...
tmp_gam <- gam(formula, data=PBDataAll2, na.action = "na.fail", method='REML')

PBDataAll2$.resid_tmp <- resid(tmp_gam)

# 2) per-TOPPID lag-1 corr (robust: exclude groups with <2 obs)
rho_by_group <- PBDataAll2 %>%
  group_by(!!sym("TOPPID")) %>%
  summarize(n = n(),
...
# Thinned data
dielmdlPB1 <- bam(DielDiff ~ s(GGBDist, k=6, bs="ts") + s(DayOfTrip, k=6, bs="ts")+ s(DayOfTrip, by=TOPPID, k = 6, bs="fs") + s(SealID, bs="re"),
                   data=PBDataAll, na.action = "na.fail", method='fREML')

# All Data no AR
dielmdlPB2a <- bam(DielDiff ~ s(GGBDist, k=6, bs="ts") + s(DayOfTrip, k=6, bs="ts")+ s(DayOfTrip, by=TOPPID, k = 6, bs="fs") + s(SealID, bs="re"),
                   data=PBDataAll2, na.action = "na.fail", method='fREML', select = TRUE)

# All Data AR with AR.start for each deployment
dielmdlPB2b <- bam(DielDiff ~ s(GGBDist, k=6, bs="ts") + s(DayOfTrip, k=6, bs="ts") + s(DayOfTrip, by=TOPPID, k=6, bs="fs") + s(SealID, bs="re"), data = PBDataAll2, 
                   AR.start = PBDataAll2$AR.start, rho = 0.378, method = "fREML")

# All Data AR with AR.start for each deployment, no factor smooth
dielmdlPB2c <- bam(DielDiff ~ s(GGBDist, k=6, bs="ts") + s(DayOfTrip, k=6, bs="ts") + s(SealID, bs="re"), data = PBDataAll2, 
                    AR.start = PBDataAll2$AR.start, rho = 0.378, method = "fREML")

# All Data, corARMA within TOPPID - this model did not converge
# dielmdlPB2d <- gamm(DielDiff ~ s(GGBDist, k=6, bs="ts") + s(DayOfTrip, k=6, bs="ts") + s(DayOfTrip, by = TOPPID, k = 6, bs="fs"),
#                     random = list(SealID = ~1),
#                     correlation = corARMA(form = ~1 | TOPPID, p = 1, q = 0),
#                     data = PBDataAll2, na.action = na.fail, method='REML')

# Intercept model
dielmdlPB2z <- gam(DielDiff ~ 1, data=PBDataAll2, na.action = "na.fail", method='REML')

# Alternate Model Summaries
summary(dielmdlPB2b)
gam.check(dielmdlPB2b)
summary(dielmdlPB2c)
gam.check(dielmdlPB2c)

# Final model and intercept model
summary(dielmdlPB2a)
summary(dielmdlPB2z)

gam.check(dielmdlPB2a)
gam.check(dielmdlPB2z)

AICc(dielmdlPB2a, dielmdlPB2z)

draw(dielmdlPB2b,select = c("GGBDist","DayOfTrip"),residuals=TRUE)

# Autocorrelation
acf(dielmdlPB2a$residuals)
pacf(dielmdlPB2a$residuals)
acf(dielmdlPB2b$residuals)
pacf(dielmdlPB2b$residuals)
acf(dielmdlPB2c$residuals)
pacf(dielmdlPB2c$residuals)

# Extract smooths for plotting
pb_obj<-plot(dielmdlPB2a,residuals=TRUE,rug=TRUE,se=TRUE,pages=0,select=1,scale=0,
         n=500,n2=40,n3=3,theta=30,phi=30,jit=FALSE,xlab=NULL,
         ylab=NULL,main=NULL,ylim=NULL,xlim=NULL,too.far=0.1,
         all.terms=FALSE,shade=TRUE,shade.col="gray80",shift=coef(dielmdlPB2a)[1],
...
pb_df <- pb_df %>%
  rename(GGBDist = x) %>%
  mutate(fit = fit + coef(dielmdlPB2a)[1])
...
PBDataAll2$resid_noAR <- resid(dielmdlPB2a)
PBDataAll2$resid_final <- resid(dielmdlPB2b)

per_rho_without <- PBDataAll2 %>%
  group_by(TOPPID) %>%
...
example_ids <- head(unique(PBDataAll2$TOPPID), 6)
par(mfrow = c(3,2))
...
plot(dielmdlPB2a, pages=1)
plot(dielmdlPB2b, pages=1)
...
```{r within-group Diagnostics}
# pooled residuals
r <- resid(dielmdlPB2c$lme, type="normalized")
acf(r)                               # overall

# per-TOPPID residual ACF (example: first 6 TOPPIDs)
PBDataAll2$fit_resid <- resid(dielmdlPB2c$lme, type="normalized")
PBDataAll2 %>%
  group_by(TOPPID) %>%
  summarize(lag1 = if(n() >= 2) cor(fit_resid[-n()], fit_resid[-1]) else NA) %>%
  slice_head(n = 6)
```
...
DielSAFZ_Dens<-ggplot()+
  ...
  geom_rug(data = PBDataAll2, mapping = aes(x = GGBDist, y = NULL), sides = "b") +
...
DielSAFZ_Dens
ggsave("Figures/SAFZ_Diel_Density_PB_V2.png", width = 8, height = 4, units = "in", dpi=600)
```
